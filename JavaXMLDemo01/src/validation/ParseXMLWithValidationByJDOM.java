 ...
 * @date 2018年12月7日
 */
public class ParseXMLWithValidationByJDOM {

	/**
	 * 解析XML文档: 不对XML文档进行验证
	 */
	@Test
	public void testParseXMLWithoutValidation() throws JDOMException, IOException {
		
		SAXBuilder builder = new SAXBuilder();
		
		Document document = builder.build("resources/xml/validation.xml");
		
		/**
		 * 解释:
		 * 	1. 因为JDOM解析器默认是不对validation.xml进行验证的,所以不论在xml是否有声明 <!DOCTYPE users SYSTEM "../dtd/validation.dtd">,都不影响
	 ...
	 */
	@Test
	public void testParseXMLWithValidation() throws JDOMException, IOException {
		SAXBuilder builder = new SAXBuilder();
		 ...
		 * 
		 * 解释:
		 * 	1. 因为我们指定了要对XML文档进行验证,所以JDOM解析XML文档的时候会去DTD文件中验证XML中的元素是否有效,最终验证为无效,所以报错	
		 */
	}
	 ...
	 */
	@Test
	public void testParseXMLWithValidationAndEntityResolver() throws JDOMException, IOException {
		SAXBuilder builder = new SAXBuilder();
		builder.setValidation(true);
		...
		Document document = builder.build("resources/xml/validation2.xml");
		
		/**
		 * 解释:
		 * 	1. 我们在XML文档中引用的外部验证文件是 ../dtd/1.dtd,但是1.dtd实际上是不存在的,所以如果我们没有手动指定外部验证文件(builder.setEntityResolver(...)),
		 * 		那么就会报FileNotFountException
		 *  2. 我们在程序中手动指定了引用外部验证文件,即使在validation2.xml中引用了一个存在的dtd文件(不论这个文件是否能够验证xml文件),
		 *  	JDOM解析器依然会按照我们在程序中指定的DTD文件来进行验证
		 */
	}
	 ...
	 */
	@Test
	public void tsetParseXMLWithValidationAndEntityResolver2() throws JDOMException, IOException {
		SAXBuilder builder = new SAXBuilder();
		builder.setValidation(true);
		builder.setEntityResolver(new EntityResolver() {
		...
		Document document = builder.build("resources/xml/validation3.xml");
	}
}
